...
import schinquirer = require("schinquirer");
...
export let conf: Conf;
try {
    conf = JSON.parse(fs.readFileSync(confPath + '/conf.json', { encoding: 'utf-8' }));
    initBot();
} catch (e) {
    const schema = JSON.parse(fs.readFileSync(confPath + '/conf.schema.json', { encoding: 'utf-8' }));
    schinquirer.prompt(schema.definitions.Conf).then(newConf => {
         conf = newConf;
         fs.writeFileSync(confPath + '/conf.json', JSON.stringify(conf), { encoding: 'utf-8' });
         initBot();
    });
}
...
function initBot() {
    bot = new Telegraf.default(conf.token);
    ...
    bot.command('ban', (ctx) => {
        if (ctx.chat.id === +conf.adminChat) {
            ...
            Promise.all([bot.telegram.sendMessage(args[1], 'You\'ve been banned: ' + args.slice(2).join(' ')), saveBan(args[1], dbHelper)]).then(() => bot.telegram.sendMessage(conf.adminChat, 'User banned correctly')
            );
        }
    });
    ...
    bot.command('warn', (ctx) => {
        if (ctx.chat.id === +conf.adminChat) {
            let args = ctx.message.text.split(' ');
            Promise.all([bot.telegram.sendMessage(args[1], 'You\'ve been warned: ' + args.slice(2).join(' ')), saveWarning(args[1], dbHelper)]).then(() => bot.telegram.sendMessage(conf.adminChat, 'User warned correctly')
            );
        }
    });
    // Bot commands end
    ...
    /// Process Reply
    function processImageReply(ctx: Context) {
    ...
    function processReply(ctx: Context, fn: (ctx: Context, userStatus: UserStatus) => void) {
        if (userStatusMap.has(ctx.from.id)) {
    ...
    function processTextReply(ctx: Context) {
        processReply(ctx, (ctx, userStatus) => {
            userStatus.extraInfo.Reply(ctx.message.text, MessageTypes.TEXT);
        });
    }
    // Process Reply end
    async function saveReportState(ctx: Context) {
    ...
    function processReport(ctx: Context) {
        const status = userStatusMap.get(ctx.from.id);
    ...
    function loadState() {
        ...
        automod.loadFromDisk(confPath);
    }
    // End
    function checkAndRemoveLastPic(ctx: Context) {
    ...
    function checkPermissionsAndExecute(ctx: Context, fn: ((ctx: Context) => any)) {
    ...
    function getUserByID(id: string) {
        return dbHelper.getUserNameByID(id);
    }
    ...
    async function resendPic(ctx: Context) {
        ...
        if (!duplicated) {
            if (!lastPic) {
                ...
                automod.registerPhoto(lastPic.originalPhoto);
                automod.registerPhoto(bestPhoto);
                lastPic = null;
            }
        }
        if (conf.resendAll) {
